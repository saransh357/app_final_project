...
function clearKey(){
  ...
  document.getElementById('pane-export').style.display='none';
...
async function exportChaosKey(){
  ...
  out.value = 'Requesting raw master key from local engine...'; 
  out.className = 'out';
  const {ok, data} = await apiFetch('/v1/export_key', {
    method:'GET',
    headers:{'Authorization':'Bearer '+_key}
  });
  if(ok){
    out.value = data.chaos_key ?? JSON.stringify(data);
  } else {
    out.value = '✗ '+(data.error||JSON.stringify(data)); 
