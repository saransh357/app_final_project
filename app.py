...
- Stable API key per user (same key returned on login unless rotated)
- POST /v1/register  — email, password, name
- POST /v1/login     — email, password → returns existing api_key
- POST /v1/rotate_key — authenticated; revokes current key, issues a new one
...
# ── bcrypt (pure-python fallback if not installed) ──────────────────────────
try:
...
def init_db():
    with app.app_context():
        ...
        # migrate: add password_hash column if missing (for existing DBs)
        try:
...
def set_relay():
    if not hmac.compare_digest(request.headers.get("X-Admin-Secret", ""), ADMIN_SECRET):
        abort(403)
...
def public_register():
    ...
    Returns the stable API key (same key on every login unless rotated).
    ...
    try:
        ...
        return jsonify({
            ...
            "note": "Save this key — it is your permanent key. Log in anytime to retrieve it."
        }), 201
    except sqlite3.IntegrityError:
...
def public_login():
    """
    Log in with email + password to retrieve the existing stable API key.
    Body: { "email": "...", "password": "..." }
    Returns: { "api_key": "ck_live_...", "tier": "...", "quota": ... }
    """
    body     = request.get_json(force=True) or {}
    email    = body.get("email", "").strip().lower()
    password = body.get("password", "").strip()
    ...
    db  = get_db()
    cust = db.execute(
        "SELECT id, name, tier, active, password_hash FROM customers WHERE email=?",
        (email,)
    ).fetchone()
    ...
    if not cust["password_hash"]:
        return jsonify({"error": "This account has no password set. Contact support."}), 401
    if not check_password(password, cust["password_hash"]):
        return jsonify({"error": "Invalid email or password"}), 401

    # Return the active (non-revoked) primary key — there should be exactly one
    key_row = db.execute(
    ...
    if not key_row:
        return jsonify({"error": "No active key found. Please contact support."}), 404

    # We cannot return the raw key (we only store hashes).
    # So we indicate the key prefix so the user recognises it, and tell them
    # to use /v1/rotate_key if they need a new one they can copy.
    quota = FREE_QUOTA_DAY if cust["tier"] == "free" else PRO_QUOTA_DAY
    return jsonify({
        ...
        "note": "Your API key is shown only once at registration. If you lost it, use POST /v1/rotate_key to issue a new one."
    }), 200
...
def rotate_key():
    """
    Revoke the current key and issue a brand-new one.
    Requires email + password in the body (not a Bearer token, because the user
    presumably doesn't have the old key anymore).
    Body: { "email": "...", "password": "..." }
    Returns: { "api_key": "ck_live_..." }
    """
    body     = request.get_json(force=True) or {}
    email    = body.get("email", "").strip().lower()
    password = body.get("password", "").strip()
    ...
    return jsonify({
        "api_key": raw_key,
        "tier":    cust["tier"],
        "note":    "Old key has been revoked. Save this new key — it won't be shown again."
    }), 201
...
def issue_key():
    """Issue an additional named key for the same account (Bearer-authenticated)."""
    auth = request.headers.get("Authorization", "")
    if not auth.startswith("Bearer "):
        return jsonify({"error": "Missing Authorization header"}), 401
    key_hash = hashlib.sha256(auth[7:].encode()).hexdigest()
...
.aw-divider{font-size:.72rem;color:var(--dust);text-align:center}
.aw-link{color:var(--lime);cursor:pointer;font-size:.78rem;text-align:center;background:none;border:none;font-family:'Outfit',sans-serif}
  ...
  <section class="hero">
    ...
    <div class="auth-widget" id="auth-widget">
      ...
      <div class="aw-body" id="pane-register">
        ...
        <div class="aw-pw-row">
          <input class="aw-input" type="password" id="reg-pw" placeholder="Password (min 6 chars)" autocomplete="new-password">
          <button class="aw-pw-toggle" onclick="togglePw('reg-pw',this)">show</button>
        </div>
      ...
      <div class="aw-body" id="pane-login" style="display:none">
        ...
        <div class="aw-pw-row">
          <input class="aw-input" type="password" id="log-pw" placeholder="Password" autocomplete="current-password">
          <button class="aw-pw-toggle" onclick="togglePw('log-pw',this)">show</button>
        </div>
    ...
    <div class="key-result" id="key-result">
      ...
      <p class="kr-note"><strong>Save this now.</strong> For security, we only store a hash — we cannot recover the raw key. Use "Rotate key" later if you ever lose it.</p>
    ...
    <!-- Login success (key already exists — can't reveal it) -->
    <div class="login-success" id="login-success">
      <div class="ls-title">✓ Logged in</div>
      <div class="ls-body" id="ls-body"></div>
      <div class="ls-actions">
        <button class="ls-btn ls-btn-primary" id="ls-paste-btn" onclick="scrollToPlayground()">Paste key below ↓</button>
...
KEY = r.json()[<span class="ts">"api_key"</span>]   <span class="tc"># save this!</span>

<span class="tc"># Login later to check your key prefix</span>
r = requests.<span class="tm">post</span>(f<span class="ts">"{BASE}/v1/login"</span>,
        json={<span class="ts">"email"</span>:<span class="ts">"you@ex.com"</span>,<span class="ts">"password"</span>:<span class="ts">"hunter2"</span>})
print(r.json()[<span class="ts">"key_prefix"</span>])   <span class="tc"># ck_live_Abc…</span>
...
print(requests.<span class="tm">post</span>(f<span class="ts">"{BASE}/v1/decrypt"</span>,
      ...
      <div class="code-card">
        ...
        <pre id="c4"><span class="tc"># Create account</span>
<span class="tm">POST</span> /v1/register
  body: { <span class="ts">"email"</span>:<span class="ts">"…"</span>, <span class="ts">"password"</span>:<span class="ts">"…"</span>, <span class="ts">"name"</span>:<span class="ts">"…"</span> }
  → { <span class="ts">"api_key"</span>: <span class="ts">"ck_live_…"</span> }  ← save this!

<span class="tc"># Log in (key prefix only — can't recover raw key)</span>
<span class="tm">POST</span> /v1/login
  body: { <span class="ts">"email"</span>:<span class="ts">"…"</span>, <span class="ts">"password"</span>:<span class="ts">"…"</span> }
  → { <span class="ts">"key_prefix"</span>: <span class="ts">"ck_live_Ab…"</span>, <span class="ts">"tier"</span>:<span class="ts">"…"</span> }

<span class="tc"># Rotate key (if lost — issues new key, revokes old)</span>
<span class="tm">POST</span> /v1/rotate_key
  body: { <span class="ts">"email"</span>:<span class="ts">"…"</span>, <span class="ts">"password"</span>:<span class="ts">"…"</span> }
  → { <span class="ts">"api_key"</span>: <span class="ts">"ck_live_…"</span> }  ← new key

<span class="tc"># Encrypt / Decrypt (Bearer token)</span>
...
<span class="tm">POST</span> /v1/decrypt  <span class="tn">[Bearer ck_live_…]</span>
  body: { <span class="ts">"ciphertext"</span>:<span class="ts">"…"</span>, <span class="ts">"nonce"</span>:<span class="ts">"…"</span>, <span class="ts">"encryption_key"</span>:<span class="ts">"…"</span> }</pre>
      </div>
    </div>
  </section>
</div>
...
(function(){
  ...
  class Particle{constructor(){this.reset(true)}reset(init){this.x=Math.random()*W;this.y=init?Math.random()*H:(Math.random()<.5?-4:H+4);this.vx=(Math.random()-.5)*SPEED;this.vy=(Math.random()*.6+.2)*SPEED*(this.y<0?1:-1);this.r=Math.random()*1.5+.4;this.life=0;this.maxLife=300+Math.random()*400;this.hue=Math.random()<.6?150:175}step(){const t=Date.now()*.0003;const nx=this.x/W*4+t,ny=this.y/H*4+t*.7;const angle=(Math.sin(nx)*Math.cos(ny))*Math.PI*2;this.vx+=Math.cos(angle)*.008;this.vy+=Math.sin(angle)*.008;this.vx*=.98;this.vy*=.98;this.x+=this.vx;this.y+=this.vy;this.life++;if(this.life>this.maxLife||this.x<-10||this.x>W+10||this.y<-10||this.y>H+10)this.reset(false)}draw(){const alpha=Math.min(this.life/60,1)*Math.min((this.maxLife-this.life)/60,1)*.6;cx.beginPath();cx.arc(this.x,this.y,this.r,0,Math.PI*2);cx.fillStyle=`hsla(${this.hue},90%,65%,${alpha})`;cx.fill()}}
...
async function pollStatus(){
  try{
    ...
    try{const s=await(await fetch('/public/stats')).json();
      document.getElementById('s-customers').textContent=s.total_customers??'—';
      document.getElementById('s-today').textContent=s.today_requests??'—';}catch(e){}
  }catch(e){}
}
...
function switchTab(tab){
  ...
  document.getElementById('pane-register').style.display=tab==='register'?'flex':'none';
  document.getElementById('pane-login').style.display=tab==='login'?'flex':'none';
}
...
async function doRegister(){
  ...
  if(!email||!email.includes('@')){setMsg(msg,'Enter a valid email','err');return}
  if(!pw||pw.length<6){setMsg(msg,'Password must be at least 6 characters','err');return}
  btn.disabled=true;setMsg(msg,'Creating account…','');
  const {ok,data}=await apiFetch('/v1/register',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({email,password:pw,name})});
  if(ok){
    saveSession({email,name:data.name||name||email.split('@')[0],tier:data.tier,quota:data.quota});
    showNewKey(data.api_key);
    updateNavUser();
    document.getElementById('auth-widget').style.display='none';
  } else {
    setMsg(msg,'✗ '+(data.error||'Registration failed'),'err');
    btn.disabled=false;
  }
}
...
async function doLogin(){
  ...
  const msg=document.getElementById('log-msg'),btn=document.getElementById('log-btn');
  if(!email||!pw){setMsg(msg,'Email and password required','err');return}
  btn.disabled=true;setMsg(msg,'Logging in…','');
  const {ok,data}=await apiFetch('/v1/login',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({email,password:pw})});
  if(ok){
    saveSession({email,name:data.name,tier:data.tier,quota:data.quota,keyPrefix:data.key_prefix,keyCreated:data.key_created});
    document.getElementById('auth-widget').style.display='none';
    showLoginSuccess(data);
    updateNavUser();
  } else {
    setMsg(msg,'✗ '+(data.error||'Login failed'),'err');
    btn.disabled=false;
  }
}
...
function doLogout(){
  ...
  document.getElementById('key-result').classList.remove('show');
  document.getElementById('auth-widget').style.display='block';
  document.getElementById('reg-btn').disabled=false;
  document.getElementById('log-btn').disabled=false;
  document.getElementById('reg-msg').textContent='Free · 100 calls/day · No credit card';
  document.getElementById('reg-msg').className='aw-msg';
  updateNavUser();
}

// ── Rotate key ────────────────────────────────────────────────────────────────
async function doRotate(){
  if(!_session){alert('You must be logged in to rotate your key.');return}
  const pw=prompt('Enter your password to confirm key rotation:');
  if(!pw)return;
  const {ok,data}=await apiFetch('/v1/rotate_key',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({email:_session.email,password:pw})});
  if(ok){
    document.getElementById('login-success').classList.remove('show');
    showNewKey(data.api_key);
    _key=data.api_key;
    sessionStorage.setItem('ck_key',_key);
  } else {
...
function updateNavUser(){
  ...
  if(_session){btn.style.display='block';btn.textContent=_session.email.split('@')[0];}
  else{btn.style.display='none';}
}
function handleNavUser(){
  if(_session)doLogout();
}

// ── UI helpers ────────────────────────────────────────────────────────────────
function showNewKey(apiKey){
  document.getElementById('kr-key-text').textContent=apiKey;
  document.getElementById('key-result').classList.add('show');
}

function showLoginSuccess(data){
  const created=data.key_created?new Date(data.key_created).toLocaleDateString():'unknown date';
  document.getElementById('ls-body').innerHTML=
    `Welcome back, <strong>${data.name||'friend'}</strong>.<br>`+
    `Tier: <span>${data.tier}</span> · Quota: <span>${data.quota} calls/day</span><br>`+
    `Active key prefix: <span>${data.key_prefix}</span> (created ${created})<br><br>`+
    `<em style="color:var(--mist);font-size:.75rem">Your raw API key was shown only once at registration. Paste it in the playground below, or rotate to get a new one.</em>`;
...
function useKeyInPlayground(){
  const k=document.getElementById('kr-key-text').textContent;
  sessionStorage.setItem('ck_key',k);_key=k;activateKeyValue(k);
  document.getElementById('playground-section').scrollIntoView({behavior:'smooth',block:'start'});
}
...
function activateKey(){
  const v=document.getElementById('pg-key-input').value.trim();if(!v)return;
  sessionStorage.setItem('ck_key',v);_key=v;activateKeyValue(v);
}
function activateKeyValue(k){
  document.getElementById('pg-key-setup').classList.add('hidden');
  ['pane-enc','pane-dec'].forEach(id=>{const el=document.getElementById(id);el.style.display='flex';el.style.flexDirection='column'});
...
function clearKey(){
  ...
  ['pane-enc','pane-dec','pane-export'].forEach(id=>document.getElementById(id).style.display='none');
...
async function apiFetch(path,opts){
  ...
  if(ct.includes('application/json')){data=await r.json()}else{const t=await r.text();const m=t.match(/<title>([^<]*)<\/title>/i);data={error:m?m[1]:'HTTP '+r.status};}
...
# Must be outside __main__ so Render/Gunicorn runs it
init_db()
